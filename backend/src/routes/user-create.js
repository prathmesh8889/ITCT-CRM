 ...
 * the same email is reused. This prevents the hidden/deleted row from causing a
 * PostgreSQL UNIQUE-email conflict while preserving that employee's historic
 * record references.
 */
  ...
  try {
    ...
    if (existing?.deleted_at) {
      ...
      await db.query(
        ...
        [req.user?.id ?? null, req.user?.name ?? "system", "User Restored", `user:${email}`, name],
      );
    } else {
      const r = await db.query(
      ...
      await db.query(
        "INSERT INTO audit_logs (user_id, user_name, action, target, detail) VALUES ($1,$2,$3,$4,$5)",
        [req.user?.id ?? null, req.user?.name ?? "system", "User Created", `user:${email}`, name],
      );
    }
