...
router.post("/users", requirePerm("employees", "create"), async (req, res, next) => {
  try {
    ...
    const role = await db.one("SELECT id, name FROM roles WHERE id = $1", [Number(b.role_id)]);
    ...
    const accessLevel = b.access_level === undefined
      ? inferAccessLevelFromRole(role.name)
      : Number(b.access_level);
    if (!isValidAccessLevel(accessLevel)) throw new HttpError(422, "access_level must be an integer from 1 to 6");
    if (role.name === "Super Admin" && accessLevel !== 1) throw new HttpError(422, "A Super Admin account must be Level 1");
    if (role.name === "Admin" && accessLevel !== 2) throw new HttpError(422, "An Admin account must be Level 2");

    const sameDepartment = !!normalizedDepartment(req.user.department)
      && normalizedDepartment(req.user.department) === normalizedDepartment(b.department);
    ...
    if (existing?.deleted_at) {
      ...
      await db.query(
        ...
        [req.user?.id ?? null, req.user?.name ?? "system", "User Restored", `user:${email}`, `${name}; L${accessLevel}; password change required`],
      );
    } else {
      const r = await db.query(
        ...
        [name, email, String(b.phone || "").trim(), hashPassword(password), b.department || "", b.designation || "",
         ...
         b.reporting_manager_id != null ? Number(b.reporting_manager_id) : null,
         b.joining_date ?? null, b.is_sales ?? false, b.active ?? true, b.color || "#0F766E", accessLevel],
      );
      row = r.rows[0];
      await db.query(
        "INSERT INTO audit_logs (user_id, user_name, action, target, detail) VALUES ($1,$2,$3,$4,$5)",
        [req.user?.id ?? null, req.user?.name ?? "system", "User Created", `user:${email}`, `${name}; L${accessLevel}; password change required`],
      );
    }
