...
export default function ChangePassword() {
  ...
  const save = async () => {
    if (!current) { toast("Enter your temporary/current password", "err"); return; }
    ...
    <div className="dot-grid flex min-h-screen items-center justify-center bg-paper p-4 dark:bg-[#0b1013] sm:p-6">
      <div className="card a-scale-in w-full max-w-md p-5 sm:p-8">
        <div className="text-center">
          ...
          <h1 className="hd mt-4 text-[20px]">Create your new password</h1>
        ...
        <div className="mt-6 space-y-4">
          <Field label="Temporary / current password" req>
          ...
          <div className="rounded-md border border-emerald-200 bg-emerald-50 p-3 text-[12px] leading-relaxed text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300">
            <span className="flex items-start gap-2"><ShieldCheck size={15} className="mt-0.5 shrink-0" /> Use at least 8 characters and avoid reusing the temporary password.</span>
          </div>
          <Btn className="w-full justify-center" loading={busy} onClick={() => void save()}><KeyRound size={15} /> Change password & continue</Btn>
        </div>
      </div>
    </div>
  );
