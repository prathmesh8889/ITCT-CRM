...
export default function Login() {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* form panel */}
      <div className="dot-grid flex flex-1 items-center justify-center bg-paper p-6 dark:bg-[#0b1013]">
        <div className="a-scale-in w-full max-w-[410px]">
          <div className="mb-6 lg:hidden">
            ...
            <div className="text-xs font-semibold uppercase tracking-[0.16em] text-ink-400">IT Cyber Technologies Pvt Ltd</div>
          </div>

          {/* mode indicator — never ambiguous about where data lives */}
          {DEMO_MODE ? (
            <div className="mb-3 flex items-center gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-[12px] font-semibold text-amber-800 dark:border-amber-800 dark:bg-amber-900/25 dark:text-amber-300">
              <FlaskConical size={14} /> DEMO MODE — browser-only sample data (VITE_DEMO_MODE=true)
            </div>
          ) : (
            <div className="mb-3 flex items-center gap-2 rounded-md border border-brand-200 bg-brand-50 px-3 py-2 text-[12px] font-semibold text-brand-700 dark:border-brand-800 dark:bg-brand-900/25 dark:text-brand-300">
              <Database size={14} /> Secure company CRM workspace · FastAPI + PostgreSQL
            </div>
          )}
          ...
          <div className="card p-6">
            ...
            <form onSubmit={submit} className="mt-5 space-y-4">
              <Field label="Email" req>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@itctcrm.in" autoFocus />
              </Field>
              ...
              {err && (
                <div className="a-fade-up rounded-md border border-red-200 bg-red-50 px-3 py-2.5 text-[12.5px] font-medium text-red-700 dark:border-red-900 dark:bg-red-900/25 dark:text-red-300">
                  ...
                  {!DEMO_MODE && err.includes("unavailable") && (
                    <div className="mt-1 font-normal text-red-500/90">Start the backend: <span className="num">cd backend · uvicorn app.main:app --reload --port 8000</span></div>
                  )}
                </div>
              )}
